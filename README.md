...
A strip of suggestions appears above the keyboard as you type, in Khmer and in
English, ranked by how common each word is. Turn it off in **Keyboard
languages**.

It is a *candidates view* rather than part of the keyboard, because the platform
can show that while the input view is hidden — which is what makes suggestions
work for someone typing on the physical keyboard.

Nothing about typing changes. Characters are committed exactly as before and
simply read back with `getTextBeforeCursor()`; only picking a suggestion edits,
by deleting the partial word and committing the whole one. The obvious
alternative — routing every keystroke through `setComposingText()` — would have
rewritten the physical keyboard's path through the key character map and made
text render as underlined-pending in every app, to no benefit here.

Khmer writes without spaces between words, so where a word begins is genuinely
ambiguous. The longest recent run of Khmer is tried as a starting point, then
progressively shorter tails of it, and the first the dictionary recognises wins.
That favours the most context that still matches something real, and it is an
approximation: proper Khmer segmentation is a research problem, usually solved
with a Viterbi or CRF model rather than a prefix lookup.

The word lists are generated by `tools/gen_dictionaries.py` into
`app/src/main/assets`, sorted by word so the app can binary-search them, each
entry carrying its frequency rank so matches can be ordered by how common they
are. Both sources are MIT licensed; see [NOTICE](NOTICE).
