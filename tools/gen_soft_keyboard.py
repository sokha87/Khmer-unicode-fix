...
# The number row lives on the symbols page, so the letter layers are three rows
# and the space it used to take goes to the suggestion strip.
...
def build(rows, language_label):
    ...
    width = '%.2f%%p' % ((100.0 - 2 * SHIFT_WIDTH) / len(rows[2]))
    out.append('    <Row>\n')
